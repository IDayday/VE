...
class HERReplayBuffer():
    ...
    def __init__(
        ...
        self._idx_to_total_obs_idx = [None] * max_size
    ...
    def add_path(self, path):
        ...
        if self._top + path_len >= self.max_size:
            ...
            for i in range(self._top, self._top + path_len):
                ...
                self._idx_to_total_obs_idx[i] = np.arange(
                    self._top, self._top + path_len
                )  
    ...
    def her_random_batch(self, batch_size):
        ...
        for i in indices:
            ...
            next_obs_i = np.sort(np.random.randint(0, num_options, 2))
            subgoal_i = next_obs_i[0]
            future_goal_i = next_obs_i[1]
            subgoal_idxs.append(possible_future_obs_idxs[subgoal_i])
            future_obs_idxs.append(possible_future_obs_idxs[future_goal_i])
        subgoal_idxs = np.array(subgoal_idxs)
        future_obs_idxs = np.array(future_obs_idxs)


        for desired_goal_key in self.desired_goal_keys:
            resampled_goals[desired_goal_key] = self._next_obs[
                desired_goal_key.replace('desired', 'achieved')
            ][future_obs_idxs]
        for goal_key in self.goal_keys:
            new_obs_dict[goal_key] = self._next_obs[goal_key][future_obs_idxs]
            new_next_obs_dict[goal_key] = self._next_obs[goal_key][future_obs_idxs]
        ...
        possible_subgoals = self._next_obs['achieved_goal'][subgoal_idxs]
        ...
        batch = {
            ...
            'possible_subgoals': possible_subgoals,
            'indices': np.array(indices).reshape(-1, 1),
        }
        return batch

    def her_final_batch(self, batch_size):
        indices = self._sample_indices(batch_size)
        resampled_goals = {
            desired_goal_key: self._next_obs[desired_goal_key][indices] for desired_goal_key in self.desired_goal_keys
        }
        ...
        for i in indices:
            ...
            final_goal_i = -1
            subgoal_idxs.append(possible_future_obs_idxs[subgoal_i])
            final_obs_idxs.append(possible_future_obs_idxs[final_goal_i])
        subgoal_idxs = np.array(subgoal_idxs)
        final_obs_idxs = np.array(final_obs_idxs)
        ...
        for goal_key in self.goal_keys:
            new_obs_dict[goal_key] = self._next_obs[goal_key][final_obs_idxs]
            new_next_obs_dict[goal_key] = self._next_obs[goal_key][final_obs_idxs]
    ...
    def buffer_random_batch(self, batch_size):
        indices = self._sample_indices(batch_size)
        ...
        # sample buffer goal
        replay_buffer_goals, idxs = self.sample_replay_buffer_states_as_goals(batch_size)
        for goal_key in self.goal_keys:
            new_obs_dict[goal_key] = replay_buffer_goals[goal_key]
            new_next_obs_dict[goal_key] = replay_buffer_goals[goal_key]
        # sample random state from same trajectory as subgoal on state or goal trajectory
        subgoal_idxs_state_trj = []
        subgoal_idxs_goal_trj = []
        for i in indices:
            possible_random_obs_idxs = self._idx_to_total_obs_idx[i]
            num_options = len(possible_random_obs_idxs)
            subgoal_i = np.random.randint(0, num_options)
            subgoal_idxs_state_trj.append(possible_random_obs_idxs[subgoal_i])
        for j in idxs:
            possible_random_obs_idxs = self._idx_to_total_obs_idx[j]
            num_options = len(possible_random_obs_idxs)
            subgoal_j = np.random.randint(0, num_options)
            subgoal_idxs_goal_trj.append(possible_random_obs_idxs[subgoal_j])  
        subgoal_idxs_state_trj = np.array(subgoal_idxs_state_trj)
        subgoal_idxs_goal_trj = np.array(subgoal_idxs_goal_trj)


        # assign replay_buffer_goals to the goals in new_obs_dict and new_next_obs_dict
        for desired_goal_key in self.desired_goal_keys:
            new_obs_dict[desired_goal_key] = replay_buffer_goals[desired_goal_key]
            new_next_obs_dict[desired_goal_key] = replay_buffer_goals[desired_goal_key]

        state_possible_subgoals = self._next_obs['achieved_goal'][subgoal_idxs_state_trj].reshape(batch_size,1,-1)
        goal_possible_subgoals = self._next_obs['achieved_goal'][subgoal_idxs_goal_trj].reshape(batch_size,1,-1)
        possible_subgoals = np.concatenate((state_possible_subgoals, goal_possible_subgoals), axis=1)
        # post process new_obs_dict and new_next_obs_dict
        new_obs_dict = postprocess_obs_dict(new_obs_dict)
        new_next_obs_dict = postprocess_obs_dict(new_next_obs_dict)
    ...
    def random_state_batch(self, batch_size, state_dim, num=1):
        sampled_state_batch = np.zeros((batch_size,num,state_dim))
        for i in range(num):
            indices = self._sample_indices(batch_size)
            new_obs_dict = {self.observation_key: self._obs[self.observation_key][indices]}
            new_obs_dict = postprocess_obs_dict(new_obs_dict)
            sample_state_batch = new_obs_dict[self.observation_key]
            sampled_state_batch[:,i,:] = sample_state_batch
        return sampled_state_batch
