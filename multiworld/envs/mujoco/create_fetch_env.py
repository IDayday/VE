from multiworld.envs.reacher import Reacher3DEnv
from multiworld.envs.pusher import PusherEnv
...
class GoalWrapper(Wrapper):
    def __init__(
        ...
        if "Pusher" in env_name:
            state_space = ob_space['observation']
            goal_space = ob_space['desired_goal']
        else:
            state_space = ob_space['observation']
            goal_space = ob_space['desired_goal']
        ...
        self.reward_shaping = "sparse"
        self.mask_goal_in_obs = mask_goal_in_obs

    def step(self, action):
        obs_dict, sparse_reward, done, info = self.env.step(action)
    ...
    def reset(self):
        ...
        out = {
            ...
            'state_desired_goal': self.env.goal,
