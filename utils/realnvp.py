...
class RealNVP(nn.Module):
    ...
    def fit(self, data, epochs=10):
        if isinstance(data, torch.FloatTensor):
            ...
            data = torch.FloatTensor(data).to(self.dev)
    ...
    def score_samples(self, x):
        z, sum_log_det_jacobian = self(torch.FloatTensor(x).to(self.dev))
...
class CouplingLayer(nn.Module):
    def __init__(self, mask_type, input_channel, dev = None):
        ...
        if '01' in self.mask_type:
            self.mask = torch.FloatTensor([[0.0]*d + (self.input_channel - d)*[1.0]]).to(self.dev)
        else:
            self.mask = torch.FloatTensor([[1.0]*d + (self.input_channel - d)*[0.0]]).to(self.dev)

    def forward(self, x, reverse=False):

        if not reverse:
            # masked half of x
            x1 = (x * self.mask).to(torch.float32)
